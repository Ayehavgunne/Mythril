...
void Lexer::skip_whitespace() {
	if (peek(-1) == '\n') {
		throw SyntaxError("Only tab characters can indent", _line_num, current_char);
	}
...
Token Lexer::get_next_token() {
	...
	// The two quote character blocks should be condensed into one
	if (current_char == '"') {
		next_char();
		while (current_char != '"') {
	...
	if (word_type == CharType::OPERATIC) {
		while (char_type == CharType::OPERATIC) {
			...
			bool cc_in_single_op = find(begin(SINGLE_OPERATORS), end(SINGLE_OPERATORS), current_char_str) != end(SINGLE_OPERATORS);
			bool word_in_single_op = find(begin(SINGLE_OPERATORS), end(SINGLE_OPERATORS), word) != end(SINGLE_OPERATORS);
	...
	if (word_type == CharType::ALPHANUMERIC) {
		...
		if (in_op) {
			...
			bool word_in_mw_op = find(begin(MULTI_WORD_OPERATORS), end(MULTI_WORD_OPERATORS), word) != end(MULTI_WORD_OPERATORS);
			bool pt_in_mw_op = find(begin(MULTI_WORD_OPERATORS), end(MULTI_WORD_OPERATORS), pt) != end(MULTI_WORD_OPERATORS);
		...
		bool in_constants = find(begin(CONSTANTS), end(CONSTANTS), word) != end(CONSTANTS);
		if (in_keywords) {
			string pt = preview_token(1).value;
			bool word_in_mw_op = find(begin(MULTI_WORD_OPERATORS), end(MULTI_WORD_OPERATORS), word) != end(MULTI_WORD_OPERATORS);
			bool pt_in_mw_op = find(begin(MULTI_WORD_OPERATORS), end(MULTI_WORD_OPERATORS), pt) != end(MULTI_WORD_OPERATORS);
	...
	if (word_type == CharType::NUMERIC) {
		while (char_type == CharType::NUMERIC || (current_char == '.' && peek(1) != '.')) {
			word += current_char;
			next_char();
			if (char_type == CharType::ALPHANUMERIC) {
				throw SyntaxError("Variables cannot start with numbers", _line_num, current_char);
			}
		}
