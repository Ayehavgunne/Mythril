...
def int_ops(compiler, op, left, right, node):
	...
	if op == PLUS:
		...
		return compiler.builder.uitofp(cmp, type_map[BOOL], 'booltmp')
	else:
...
def float_ops(compiler, op, left, right, node):
	if op == PLUS:
		...
		return compiler.builder.udiv(compiler.builder.fptosi(left, ir.IntType(64)),
...
def cast_ops(compiler, left, right, node):
	...
	if cast_type == I64:
		if orig_type == DOUBLE:
			cast = compiler.builder.fptoui(left, type_map[INT])
			return cast
	elif cast_type == DOUBLE:
		if orig_type == I64:
			cast = compiler.builder.uitofp(left, type_map[DEC])
			return cast
